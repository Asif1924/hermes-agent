...
def get_platform_defaults(platform_key: str) -> dict:
    """Return a copy of the built-in display defaults for a platform.

    Returns a new dict so callers can mutate it without affecting shared tier dicts.
    Unknown platforms get a copy of the global defaults.
    """
    plat_defaults = _PLATFORM_DEFAULTS.get(platform_key)
    if plat_defaults:
        return dict(plat_defaults)
    return dict(_GLOBAL_DEFAULTS)


def get_effective_display(config: dict, platform_key: str) -> dict:
    """Return a complete dict of resolved display settings for a platform.

    Merges platform overrides, global user settings, built-in platform
    defaults, and global defaults — same resolution order as
    ``resolve_display_setting()`` — for all overrideable keys.
    """
    return {
        key: resolve_display_setting(config, platform_key, key, _GLOBAL_DEFAULTS.get(key))
