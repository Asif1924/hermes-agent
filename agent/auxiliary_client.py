...
def _read_codex_access_token() -> Optional[str]:
    ...
    Reads directly from the Hermes auth store (HERMES_HOME/auth.json) when the
    file exists, to avoid triggering pool seeding / auto-import from
    ~/.codex/auth.json.  Only falls through to the pool / legacy path when the
    auth store file is absent (first-time setup, tests that patch
    _select_pool_entry, etc.).
    """
    # Step 1: Direct auth-store read (avoids pool auto-import from ~/.codex/).
    # When the auth file exists it is the authoritative source:
    #   - state present + valid token  -> return token
    #   - state present + expired token -> return None
    #   - state absent (explicitly not configured) -> return None
    try:
        from hermes_constants import get_hermes_home as _ghh
        from hermes_cli.auth import _load_auth_store as _las, _load_provider_state as _lps
        _auth_file = _ghh() / "auth.json"
        if _auth_file.is_file():
            _store = _las()
            _state = _lps(_store, "openai-codex")
            if not isinstance(_state, dict):
                # File exists but no openai-codex entry — explicitly not configured.
                return None
            _tokens = _state.get("tokens")
            if not isinstance(_tokens, dict):
                return None
            _token = str(_tokens.get("access_token", "") or "").strip()
            if not _token:
                return None
            # Check JWT expiry
            try:
                ...
                if _exp and time.time() > _exp:
                    logger.debug("Codex access token expired (exp=%s), skipping", _exp)
                    return None
            except Exception:
                pass  # Non-JWT token or decode error — use as-is
            return _token
    except Exception:
        pass  # Auth store unreadable — fall through to pool / legacy path

    # Step 2: Pool path (for callers/tests that patch _select_pool_entry directly,
    # and for edge cases where the auth file does not exist yet).
    ...
    # Step 3: Legacy _read_codex_tokens fallback (handles test scenarios that
    # patch this function, and any remaining edge cases).
    try:
