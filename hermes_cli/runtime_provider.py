...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(providers, dict):
        for ep_name, entry in providers.items():
            ...
            # api_key may be specified directly in the entry or via key_env
            direct_api_key = str(entry.get("api_key", "") or "").strip()
            effective_api_key = resolved_api_key or direct_api_key
            ...
            if requested_norm in {ep_name, name_norm, f"custom:{name_norm}"}:
                ...
                if base_url:
                    ...
                    # Support "transport" as an alias for "api_mode" in providers dict
                    transport = _parse_api_mode(entry.get("transport") or entry.get("api_mode"))
                    if transport:
                        result["api_mode"] = transport
                    return result
